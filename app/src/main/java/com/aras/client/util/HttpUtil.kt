...
object HttpUtil {
     ...
     */
    /** Body plus the subscription metadata headers we care about. */
    class FetchedSubscription(val body: String, val headers: Map<String, String>)

    /** Headers captured for subscription metadata parsing. */
    val SUB_INFO_HEADERS = listOf(
    ...
    /**
     * Fetches a subscription URL and returns the body together with the
     * metadata response headers (traffic/expiry, announcements, profile title).
     */
    fun getSubscriptionWithHeaders(request: UrlContentRequest): FetchedSubscription {
        var currentUrl: String? = request.url
        var redirects = 0
        val maxRedirects = 3
        ...
        while (redirects++ < maxRedirects) {
            ...
            val finalUserAgent = if (request.userAgent.isNullOrBlank()) {
                "ArasClient/${BuildConfig.VERSION_NAME}"
            } else {
                request.userAgent
            }
            val requestBuilder = Request.Builder()
            ...
            client.newCall(requestBuilder.build()).execute().use { response ->
                when {
                    ...
                    response.isSuccessful -> {
                        val headers = SUB_INFO_HEADERS.mapNotNull { key ->
                            response.header(key)?.let { key to it }
                        }.toMap()
                        return FetchedSubscription(response.body?.string() ?: "", headers)
                    }
